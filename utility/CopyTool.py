...
class CopyTool(object):
  ...
  def __init__(self, analysis_name, configuration, force_all):
    ...
    self.path_batch_scripts       = os.path.join( self.path_working_directory, 'results', analysis_name, '_1_copy_batch')
    self.path_batch_templates     = os.path.join( self.path_working_directory, 'utility', 'templates', 'copy_batch')
  ...
  def get_list_of_samples_from_sources(self):
    ...
    for _l in self.list_of_locations:
      ...
      for _s in self.wrapper_gfal_ls( _path ).splitlines():
        self.list_of_samples_from_sources[_s.split(' ')[-1].strip('\t')] = _l
  ...
  def copy_files_single_sample_batch(self, sample):

    MiscTool.Print('python_info', '\nCalled copy_files_single_sample_batch function.')
  ...
  def copy_files_all_samples_from_config_batch(self):

    MiscTool.Print('python_info', '\nCalled copy_files_all_samples_from_config_batch function.')
    
    # Loop over all samples
    for _s in self.list_of_samples_from_config:
      
      self.copy_files_single_sample_batch(_s)
